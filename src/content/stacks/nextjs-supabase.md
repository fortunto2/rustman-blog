...
framework_release:
  # What 16.3 changed, and why each line matters for how we actually work.
  dev_memory: "Up to 90% less memory in dev. The one that matters most here — several dev servers plus agents on one machine stopped being the constraint. Upgrade before blaming the laptop."
  speed: "Faster builds, type-checking and rendering across the board. Re-time the pre-commit gate after upgrading; the oxlint/tsgo opt-ins in fast_tooling may be less urgent than they were."
  agent_tooling: "Improved tooling for AI agents — the framework now meets the harness halfway. Check what it exposes before hand-rolling an agent-readable layer (see the llms.txt / markdown notes below)."
  error_boundaries: "Custom error boundaries — scope a failure to the subtree that caused it instead of the route-level error.tsx catch-all."
  instant_navigations: "Instant Navigations — SPA-grade responsiveness on App Router transitions, no client-side router swap needed."
...
fast_tooling:
  # Native (Rust/Go) rewrites of the JS toolchain — 10-100x speedups, compound with team size.
  # Defaults above (eslint/prettier/tsc) stay canonical; add these as opt-in fast gates.
  # Rust > Go when equivalent. Ref: wiki native-frontend-toolchain.
  note: "Opt-in fast gates. Adopt as default once each leaves RC / covers all needed rules."
  lint: "oxlint (oxc, Rust) — pre-commit gate, ~8x ESLint local / 3.6x CI, RAM 511→212MB. Keep ESLint for Next-specific plugin rules (@next/eslint-plugin-next). Run: pnpm dlx oxlint@latest"
  type_check: "tsgo --noEmit (TSGO — Microsoft Go rewrite of tsc) — ~10x faster, RAM −83%. Release Candidate as of mid-2026; make default once stable. Install: @typescript/native-preview"
  react_compiler: "reactCompiler: true in next.config (React 19, already in stack). Removes manual memo/useMemo/useCallback; 7mo+ prod-proven, cleaner code, lower junior barrier"
  api_codegen: "OpenAPI/Swagger backends → generate the client, don't hand-write types: swagger-typescript-api OR openapi-typescript. 'Compilation becomes an integration test' — contract drift fails at tsc, not prod"
...
notes: |
  ...
  - Fast toolchain (opt-in, see fast_tooling): oxlint pre-commit gate, tsgo (when stable), React Compiler (reactCompiler: true), OpenAPI codegen. Native Rust/Go rewrites, 10-100x. Wiki: native-frontend-toolchain
  ...
  - Next.js 16.3: dev memory −90%, faster build/typecheck/render, custom error boundaries, Instant Navigations, better AI-agent tooling (see framework_release)
